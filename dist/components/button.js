export class Button {
    constructor(ctx, x, y, w, h, label, onClick, options) {
        this.ctx = ctx;
        this.x = x;
        this.y = y;
        this.w = w;
    ...
    handleMouseDown(offset_x, offset_y) {
        if (this.contains(offset_x, offset_y)) {
            this.is_pressed = true;
            this.target_scale = 0.95;
        }
    }
    handleMouseUp(offset_x, offset_y) {
    ...
    contains(offset_x, offset_y) {
        return (offset_x >= this.x &&
